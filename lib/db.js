...
// Safe migration for last_login_at column if database was created previously
try {
...
export function createProduct(productData) {
  ...
  let generatedId = id || model.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
  if (!generatedId) generatedId = 'phone-' + Date.now();

  // Check if a product with this ID already exists
  const existing = db.prepare('SELECT id, is_active FROM products WHERE id = ?').get(generatedId);
  if (existing) {
    if (existing.is_active === 0) {
      // Reactivate and update existing inactive record
      const updateStmt = db.prepare(`
        UPDATE products SET
          brand = ?, model = ?, price = ?, mrp = ?, discount = ?,
          ram = ?, storage = ?, display = ?, camera = ?, battery = ?,
          processor = ?, stock = ?, rating = ?, badge = ?, image = ?,
          is_active = 1, updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
      `);
      updateStmt.run(
        brand, model, Number(price), Number(mrp || price), discount,
        ram, storage, display, camera, battery, processor,
        Number(stock), Number(rating), badge, image, generatedId
      );
      logActivity('product_added', `New Phone Added: ${model}`, `Brand: ${brand}, Price: ₹${price}, Stock: ${stock}`);
      return getProductById(generatedId);
    } else {
      // Active product exists with this ID; make ID unique by appending unique slug
      let candidateId = generatedId;
      let counter = 1;
      const checkStmt = db.prepare('SELECT id FROM products WHERE id = ?');
      while (checkStmt.get(candidateId)) {
        candidateId = `${generatedId}-${Date.now().toString(36).slice(-4)}${counter > 1 ? '-' + counter : ''}`;
        counter++;
      }
