...
export class RagEngine {
  ...
  loadKnowledgeBase() {
    ...
    // 1. Ingest Products Catalog (SQLite Database first, fallback to products.json)
    let products = [];
    try {
      products = getAllProducts();
    } catch (e) {
      console.warn('Could not read products from SQLite:', e.message);
    }
    ...
    for (const p of products) {
      ...
      const specsText = [
        ...
        `Price: ₹${Number(p.price).toLocaleString('en-IN')} (MRP: ₹${Number(p.mrp || p.price).toLocaleString('en-IN')})`,
        `RAM & Storage: ${p.ram}, ${p.storage}`,
        `Display: ${p.display || '6.7" AMOLED'}`,
        `Camera: ${p.camera || '50MP OIS'}`,
        `Battery & Charging: ${p.battery || '5000 mAh'}`,
        `Processor: ${p.processor || 'Octa-Core 5G'}`,
        `Key Features: ${keyFeatures.join('; ')}`,
        `Stock Status: ${p.stock > 0 ? `${p.stock} units in stock` : 'Out of Stock'}`,
        `Customer Rating: ${p.rating || 4.5}/5`
      ].join('\n');
      ...
      this.chunks.push({
        ...
        metadata: {
          ...
          ram: p.ram,
          ram_size_gb: p.ram_size_gb || (parseInt(p.ram, 10) || 8),
          camera: p.camera,
          battery: p.battery,
  ...
  retrieve(query, topK = 4) {
    if (!this.isInitialized || Date.now() - (this.lastLoadedAt || 0) > 10000) {
      this.init();
